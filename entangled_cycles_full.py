...
u=np.array(v[1])
w=np.array(v[2])
# x=np.array(z_carrier_alpha[0])
# y=np.array(z_carrier_beta[0])
# z=np.array(np.log(np.abs(z_carrier[0])))
...
def update_points(num, x, y, z, points):
    ...
    new_x=z_carrier_alpha +np.random.normal(1,0.1, size=(len(x),))
    new_y=z_carrier_beta +np.random.normal(1,0.1, size=(len(y),))
    new_z=np.log(np.abs(z_carrier)) +np.random.normal(1,0.1, size=(len(z),))
    # update properties
